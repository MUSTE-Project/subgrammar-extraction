...
recreateFromExamples g_r lang_r g_0 examples maxSubtreeSize maxMergedPerTree ofun =
  do
    ...
    let problem = forestsToProblem maxSubtreeSize (Just maxMergedPerTree) forests [] ofun
...
compareTreebank g_r lang_r treeBank maxSubtreeSize maxMergedPerTree ofun =
  do
    let
      ...
      problem = forestsToProblem maxSubtreeSize (Just maxMergedPerTree) forests [] ofun
    solution <- solve problem
    g' <- generateGrammar g_r default_rgl_path default_rgl_subdirs solution False
